...
#             df[column] = df[column].astype('category')
            ...
            if x == 1 or isinstance(kwargs, dict):
                cols[column][1] = float(cols[column][1][0]) + round(
                    float(df[column].memory_usage(deep=True)/ 1024**2),4)
                if x ==1:
    ...
    if isinstance(kwargs, dict) and x == 0:
        ...
        pd.DataFrame(df.info(memory_usage='deep'))
        print(f'{realtime()[-13:]}>> from = {a} MB\n'f'{realtime()[-13:]}>> to = {b} MB    (include "object" type on {cnt} MB)\n'f'{realtime()[-13:]}>> profit: {a-b} MB\n'f'\n'f'{[*cols.items()]}')
